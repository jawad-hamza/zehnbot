...
const input: React.CSSProperties = { padding: "8px 11px", border: "1px solid var(--border-strong)", borderRadius: 8, fontSize: 13, outline: "none", fontFamily: "inherit" };
const label: React.CSSProperties = { fontSize: 12, fontWeight: 600, color: "var(--fg-2)", display: "flex", flexDirection: "column", gap: 4 };
const card: React.CSSProperties = { background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 12, padding: "18px 22px", marginBottom: 12 };
...
export default function TenantsPage() {
  ...
  if (me && me.role !== "superadmin") return <Navigate to="/bots" replace />;
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
        ...
        <button onClick={() => setShowCreate((v) => !v)} style={{ ...smallBtn("var(--primary)", "var(--surface)"), padding: "9px 20px", fontSize: 14 }}>
          {showCreate ? "Cancel" : "+ New Tenant"}
        </button>
      </div>

      {error && <p style={{ color: "var(--danger)", fontSize: 13, marginBottom: 16, whiteSpace: "pre-wrap" }}>{error}</p>}
      ...
      {!loading && tenants.length === 0 && <p style={{ color: "var(--muted-fg)" }}>No tenants yet. Create your first customer above.</p>}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
            <div style={{ flex: 1, minWidth: 220 }}>
              <div style={{ fontWeight: 600, fontSize: 15 }}>
                {t.name}
                <span style={{ marginLeft: 8, background: "var(--accent-soft)", color: "var(--accent-text)", padding: "1px 8px", borderRadius: 999, fontSize: 11, fontWeight: 700, textTransform: "uppercase" }}>{t.plan}</span>
                {!t.is_active && <span style={{ marginLeft: 6, background: "var(--danger-soft)", color: "var(--danger)", padding: "1px 8px", borderRadius: 999, fontSize: 11, fontWeight: 700 }}>SUSPENDED</span>}
              </div>
              <div style={{ color: "var(--muted-fg)", fontSize: 13, marginTop: 2 }}>{t.users.map((u) => u.email).join(", ") || "no logins"}</div>
            </div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button style={smallBtn("var(--surface-3)", "var(--fg)")} onClick={() => setEditingId(editingId === t.id ? null : t.id)}>{editingId === t.id ? "Close" : "Manage"}</button>
              <button
                style={smallBtn(t.is_active ? "var(--warning-soft)" : "var(--success-soft)", t.is_active ? "var(--warning)" : "var(--success)")}
...
function Stat({ title, value, warn }: { title: string; value: string; warn?: boolean }) {
  return (
    <div style={{ minWidth: 110 }}>
      <div style={{ fontSize: 11, color: "var(--subtle-fg)" }}>{title}</div>
      <div style={{ fontSize: 14, fontWeight: 600, color: warn ? "var(--danger)" : "var(--fg)" }}>{value}</div>
    </div>
  );
}
...
function CreateTenantForm({ plans, onCreate }: { plans: Plan[]; onCreate: (body: object) => void }) {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onCreate(form); }} style={{ ...card, background: "var(--surface-2)", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
      <label style={label}>Company name *<input style={input} value={form.name} onChange={set("name")} required minLength={2} placeholder="Example Ltd" /></label>
      ...
      <label style={label}>Owner login (email) *<input style={input} value={form.owner_email} onChange={set("owner_email")} required placeholder="owner@example.com" autoComplete="off" /></label>
      ...
      <button type="submit" style={{ ...smallBtn("var(--primary)", "var(--surface)"), padding: "9px 20px", fontSize: 13, justifySelf: "start" }}>Create Tenant</button>
    </form>
  );
}
  ...
  return (
    <div style={{ borderTop: "1px solid var(--border)", marginTop: 16, paddingTop: 16 }}>
      ...
      >
        ...
        <button type="submit" style={{ ...smallBtn("var(--primary)", "var(--surface)"), padding: "9px 16px" }}>Save</button>
      </form>

      <div style={{ fontSize: 12, fontWeight: 700, color: "var(--fg-2)", margin: "18px 0 8px" }}>Logins</div>
      {tenant.users.map((u) => (
        <div key={u.id} style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 13, marginBottom: 6 }}>
          <span style={{ flex: 1, color: "var(--fg-2)" }}>{u.email}</span>
          <button type="button" style={smallBtn("var(--surface-3)", "var(--fg)")} onClick={() => resetPassword(u.id, u.email)}>Reset password</button>
        </div>
      ))}
      <form
      ...
      >
        ...
        <button type="submit" style={smallBtn("var(--surface-3)", "var(--fg)")}>Add login</button>
      </form>

      <button type="button" onClick={onDelete} style={{ marginTop: 18, background: "none", border: "1px solid var(--danger-border)", color: "var(--danger)", borderRadius: 6, padding: "6px 14px", cursor: "pointer", fontSize: 12 }}>
