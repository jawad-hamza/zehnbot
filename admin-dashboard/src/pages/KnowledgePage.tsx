...
export default function KnowledgePage() {
  ...
  const box: React.CSSProperties = { background: "var(--surface-2)", border: "1px solid var(--border)", borderRadius: 12, padding: 18, marginBottom: 18 };
  const box_title: React.CSSProperties = { fontSize: 13, fontWeight: 700, color: "var(--fg-2)", marginBottom: 6 };
  const box_hint: React.CSSProperties = { fontSize: 12, color: "var(--muted-fg)", marginBottom: 10 };
  const inputBase: React.CSSProperties = { padding: "9px 12px", border: "1px solid var(--border-strong)", borderRadius: 8, fontSize: 13, outline: "none", fontFamily: "inherit" };
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 24 }}>
        ...
        <span style={{ color: "var(--muted-fg)", fontSize: 13 }}>{chunks.length} chunks · {sources.length} source(s)</span>
      </div>
      ...
      <div style={{ maxWidth: 720 }}>
            ...
            <ul style={{ margin: 0, padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: 6 }}>
              {sources.map(([label, count]) => (
                <li key={label} style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 13 }}>
                  <span style={{ background: "var(--surface-3)", color: "var(--fg-2)", padding: "2px 8px", borderRadius: 4, fontSize: 11, fontWeight: 600, minWidth: 40, textAlign: "center" }}>{count}</span>
                  <span style={{ color: "var(--fg-2)", wordBreak: "break-all", flex: 1 }}>{label}</span>
                  <button type="button" onClick={() => handleDeleteSource(label)} title="Delete this source" style={{ background: "none", border: "none", color: "var(--danger)", cursor: "pointer", fontSize: 12 }}>
                    Delete
                  </button>
                </li>
              ))}
            </ul>
            <button type="button" onClick={handleDelete} style={{ marginTop: 12, background: "none", border: "1px solid var(--danger-border)", color: "var(--danger)", borderRadius: 6, padding: "6px 14px", cursor: "pointer", fontSize: 12 }}>
              Delete all
            </button>
          </div>
        )}
        ...
        <div style={box}>
          ...
          <form onSubmit={handleCrawl} style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            <button type="submit" disabled={crawling || !crawlUrl.trim()} style={{ background: "var(--primary)", color: "var(--on-primary)", border: "none", borderRadius: 8, padding: "9px 18px", cursor: "pointer", fontSize: 13, fontWeight: 600, alignSelf: "flex-start", opacity: crawling ? 0.6 : 1 }}>
              {crawling ? "Crawling…" : "Crawl Site"}
            </button>
          </form>
          {crawlMsg && <div style={{ fontSize: 12, color: crawlMsg.startsWith("Crawled") ? "var(--success)" : "var(--danger)", marginTop: 8, whiteSpace: "pre-wrap" }}>{crawlMsg}</div>}
        </div>
        ...
        <div style={box}>
          ...
          <input
            ...
            style={{ fontSize: 13 }}
          />
          {uploading && <div style={{ fontSize: 12, color: "var(--muted-fg)", marginTop: 8 }}>Uploading…</div>}
          {fileMsg && <div style={{ fontSize: 12, color: fileMsg.includes("failed") ? "var(--danger)" : "var(--success)", marginTop: 8, whiteSpace: "pre-wrap" }}>{fileMsg}</div>}
        </div>
        ...
        <div style={box}>
          ...
          <form onSubmit={handleIngestUrls} style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            />
            <button type="submit" disabled={ingestingUrl || !urls.trim()} style={{ background: "var(--accent-text)", color: "var(--on-primary)", border: "none", borderRadius: 8, padding: "9px 18px", cursor: "pointer", fontSize: 13, fontWeight: 600, alignSelf: "flex-start", opacity: ingestingUrl ? 0.6 : 1 }}>
              {ingestingUrl ? "Fetching…" : "Fetch & Add"}
            </button>
          </form>
          {urlMsg && <div style={{ fontSize: 12, color: urlMsg.startsWith("Added") && !urlMsg.includes("failed") ? "var(--success)" : "var(--danger)", marginTop: 8, whiteSpace: "pre-wrap" }}>{urlMsg}</div>}
        </div>

        {/* Manual text — one source per label */}
        <form onSubmit={handleSave} style={{ ...box, background: "var(--surface)" }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 8 }}>
            <span style={{ fontSize: 12, color: "var(--fg-2)" }}>Source label:</span>
          ...
          <div style={{ display: "flex", gap: 12, alignItems: "center", marginTop: 10 }}>
            <button type="submit" disabled={saving || !rawText.trim()} style={{ background: "var(--primary)", color: "var(--on-primary)", border: "none", borderRadius: 8, padding: "9px 22px", cursor: "pointer", fontSize: 14, fontWeight: 600, opacity: saving || !rawText.trim() ? 0.6 : 1 }}>
              {saving ? "Saving…" : "Save Source"}
            </button>
            {msg && <span style={{ fontSize: 13, color: msg.startsWith("Saved") || msg.startsWith("Knowledge deleted") ? "var(--success)" : "var(--danger)" }}>{msg}</span>}
          </div>
        </form>
      </div>
    </div>
